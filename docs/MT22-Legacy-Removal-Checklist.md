...
- [x] `has_legacy_single_task()` 與 `_get_legacy_task_if_exists()` 已被正式標記為內部 deprecated，只存在於 tasks.py 內部（供 get_migration_status + doctor）與受保護的歷史測試中使用。
- [x] `get_task_migration_status()` 已成為主要的（且足夠）診斷工具。
- [x] `tests/test_task.py` 已明確標記為「僅用於歷史相容驗證」+ pytest.importorskip 保護，收集時在無 task.py 環境安全跳過。
...
## 建議執行順序（已完成大部，2026-05）

1. [x] 完成 `test_tasks.py` 的 fixture 清理 + 最終 legacy 引用清除（死測刪除、mangled 修復、強化 docstring 說明僅用於遷移驗證）。
2. [x] 補齊/保留 `test_doctor.py` 對各種遷移狀態的覆蓋（legacy_only/mixed/multi_only 仍正確回報，診斷能力保留）。
...
**記錄時間**：2026 + task.py 移除 + test_tasks.py 最終清理 + 驗證通過  
**負責人**：Grok（逐一推進，符合 CLAUDE §6 rtk/ruff/驗證紀律）  
**更新**：
- cli.py 三處 legacy if 分支已移除。
- _format_legacy 函式刪除。
- _get_legacy 從 tasks.py 移除（最後 src dep 清除）。
- task.py 模組已 git rm。
- test_task.py 更新為「僅歷史相容驗證」+ importorskip（收集時安全跳過）。
