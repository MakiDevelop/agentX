...
> 僅限人類本人執行。Agent / Codex 可提供下列指令草稿，但不得代為執行
> `rm` / `rmdir` / `unlink` 或等價刪除操作。
...
舊系統移除後一段時間，建議團隊可以考慮以下做法：

- 定期執行 `agentx doctor`，確認是否還有任何殘留的舊 `task.json`。
- 在團隊 onboarding 文件中加入「我們已完全使用多任務清單」這段說明。
- 如果有人還是習慣舊的單一任務思考方式，可以引導他們使用多任務的「進行中上限」概念來管理專注度。
- 建議在 3–6 個月後，考慮把所有歷史 `task.json.bak.*` 做一次集中清理或歸檔。

---

## 移除後的團隊溝通建議

如果團隊要推動舊系統完全退場，以下是一些實務做法：

- **提前 2-4 週溝通**：在 Slack / 會議提前告知「舊單一任務系統將在 v1.0.0 完全移除」。
- **提供過渡指引**：直接分享本 Migration Guide 的「使用者遷移建議流程」章節。
- **降低阻力**：可先讓 `agentx doctor` 繼續保留對舊資料的診斷能力 1-2 個版本。
- **收集反饋**：在移除後 1 個月內，主動問團隊是否有遇到舊系統相關的困擾。
