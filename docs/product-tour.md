...
需要執行命令前先做政策預檢時，用 `agentx command-plan COMMAND --json` 取得 `agentx.command_plan.v1`；它判斷 allowlist、YELLOW approval、top-level agentX CLI capability、headless agent posture 與 destructive blockers，不執行命令。
...
外部 runner 可用 `agentx workflows --json` 取得常用 recipe catalog；每個 workflow 會提供含 `kind` 的 `steps`、可直接處理的 `commands`，且 top-level `agentx` step 會內嵌 `command_plan`，需要單一路徑時用 `agentx workflows headless --json` 或 `agentx workflows audit --json`。
