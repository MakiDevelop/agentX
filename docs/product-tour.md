...
`/sessions` 會顯示每輪 transcript 的 approval receipt counts，方便快速看出是否有 YELLOW tool 被拒絕；`/transcript approvals latest` 或 `/transcript approvals SESSION` 可以列出本輪、最近上一輪或指定 session 的 approval receipts。
