...
- `handoff_summary` 會用 deterministic runtime state 輸出 status、needs_handoff、failing_tools、pending_verifies、last_error、task_counts、next_steps；若有 session_path 也會附 `resume_session` 與 `resume_command`，不額外呼叫模型。
