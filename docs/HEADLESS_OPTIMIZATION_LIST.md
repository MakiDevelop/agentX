...
- [x] `agentx patch-check --json` 會提供 top-level `recommended_command` / `recommended_kind` / `recommended_risk`，成功時推薦 `/apply PATCH`，失敗時推薦修 patch blocker 後重跑 patch-check。
...
- [x] `agentx workflows memory --json` 會提供 AMH handoff recipe（read → dry-run write → explicit write），讓 runner 能用同一條 route 管理記憶交接。
...
- [x] AMH memory workflow 已有 runner smoke 覆蓋 `workflow-run --result-output` → `artifacts.workflow_chain` → `next workflow-resume` → `workflow-resume --dry-run` → `gate doctor workflow_artifact_health` 的 dry-run 串接。
...
- [x] `agentx reliability-suite --json` 已提供 local-only recorded backend suite，建立 fixture repos 並評估 headless artifacts、tool-call count、termination、expected files、`artifacts`、`next`、`gate` 與 recovery recommendation posture。
...
- [x] `agentx commit-plan --json` 會提供 top-level `recommended_command` / `recommended_kind` / `recommended_risk`，與 review/gate 的提交前 runner 摘要一致。
...
- [x] `agentx handoff-inspect --briefing-output PATH` 可把 deterministic 接手摘要寫成 workspace 內 Markdown artifact，拒絕覆寫與 workspace escape；可搭配 `--next-prompt-file PATH` 讓 resume command 指向同一份 briefing。
