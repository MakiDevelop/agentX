...
| Maki 混合要求「資源地圖+家庭AI設施/VPS地圖」 | `/infra resource-bundle` 或 `/infra 資源地圖+家庭AI設施/VPS地圖` | 同時載入 resource map、家庭 AI 專章、VPS 專章 |

當 Maki 要求「資源地圖」「家庭 AI 設施地圖」「VPS 地圖」時，agentX 的交付物應包含：
- repo 內可讀索引：`docs/RESOURCE_MAPS.md`，說明 source、alias、路由、停止條件。
- runtime 可查入口：`/infra home`、`/infra vps`、`/infra resource-bundle`、`/infra all`，且標明 read-only。
