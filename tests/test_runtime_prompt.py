...
def test_agent_prompt_states_ssh_limit_and_tool_evidence_rule():
    ...
    assert (
        "arbitrary shell commands" in AGENT_SYSTEM_PROMPT
        or "You cannot run arbitrary" in AGENT_SYSTEM_PROMPT
    )
    ...
    assert (
        "engineering agent" in AGENT_SYSTEM_PROMPT.lower()
        or "local engineering agent" in AGENT_SYSTEM_PROMPT.lower()
    )
    assert (
        "AMH means Agent Memory Hall / local long-term memory, not web search"
