...
def test_check_task_migration_legacy_with_invalid_multi_tasks_structure(tmp_path: Path) -> None:
    """有舊任務，但新的 tasks.json 結構錯誤（非 list）時，應能安全回報狀態。"""
    legacy_dir = tmp_path / ".agentx"
    legacy_dir.mkdir()
    (legacy_dir / "task.json").write_text('{"title":"舊任務","status":"active"}', encoding="utf-8")

    # 建立結構錯誤的 tasks.json（不是陣列）
    (legacy_dir / "tasks.json").write_text('{"not": "a list"}', encoding="utf-8")
...
def test_check_task_migration_mixed_with_in_progress_task(tmp_path: Path) -> None:
    """有舊任務 + 新任務狀態為 in_progress 時，應正確回報 mixed 且 counts 合理。"""
    legacy_dir = tmp_path / ".agentx"
    legacy_dir.mkdir()
    (legacy_dir / "task.json").write_text('{"title":"舊任務","status":"active"}', encoding="utf-8")
    ...
    assert "mixed" in detail
    assert "legacy=True" in detail
    assert "tasks=2" in detail


def test_check_task_migration_legacy_with_invalid_status_in_multi(tmp_path: Path) -> None:
    """有舊任務 + 新任務有未知 status 時，應能正確回報 mixed 而不崩潰。"""
    legacy_dir = tmp_path / ".agentx"
    legacy_dir.mkdir()
    (legacy_dir / "task.json").write_text('{"title":"舊任務","status":"active"}', encoding="utf-8")
    ...
    assert "mixed" in detail
    assert "legacy=True" in detail
    # 未知 status 應被正規化處理，不影響整體判斷
    assert "tasks=2" in detail or "tasks=" in detail
...
def test_check_task_migration_legacy_with_many_multi_tasks(tmp_path: Path) -> None:
    """有舊任務 + 新任務很多筆時，應正確回報 mixed 且 tasks 數量正確。"""
    legacy_dir = tmp_path / ".agentx"
    legacy_dir.mkdir()
    (legacy_dir / "task.json").write_text('{"title":"舊任務","status":"active"}', encoding="utf-8")
...
def test_check_task_migration_legacy_with_extreme_mixed_state(tmp_path: Path) -> None:
    """有舊任務 + 新任務同時有 in_progress 和大量 done 時，應正確回報 mixed。"""
    legacy_dir = tmp_path / ".agentx"
    legacy_dir.mkdir()
    (legacy_dir / "task.json").write_text('{"title":"舊任務","status":"active"}', encoding="utf-8")
    ...
    assert "mixed" in detail
    assert "legacy=True" in detail
    assert "tasks=6" in detail


def test_check_task_migration_legacy_with_very_long_title_in_multi(tmp_path: Path) -> None:
    """有舊任務 + 新任務 title 極長時，應能正確回報 mixed 而不受影響。"""
    legacy_dir = tmp_path / ".agentx"
    legacy_dir.mkdir()
    (legacy_dir / "task.json").write_text('{"title":"舊任務","status":"active"}', encoding="utf-8")
