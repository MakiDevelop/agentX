...
| `/infra [all\|quick\|project\|resource\|home\|vps]` | 讀取專案/資源地圖；`home`/`vps` 會抽取家庭 AI 設施與 VPS 章節，也支援 `家庭AI設施`、`VPS地圖`、`外網主機`、`資源地圖+家庭AI設施/VPS地圖` 等中文 alias（read-only）。地圖分層與使用規則見 [`docs/RESOURCE_MAPS.md`](docs/RESOURCE_MAPS.md) |
