...
agentx patch-check patches/fix.patch --json --fail-on-blocker
agentx command-plan "uv run pytest -q" --json
agentx command-plan "npm test" --json
agentx command-plan "python manage.py migrate" --json --fail-on-blocker
agentx review --json
agentx review --json --fail-on-blocker
agentx commit-plan --message "新增功能" --json
agentx commit-plan --message "新增功能" --json --fail-on-blocker
agentx gate --json
...
agentx memory-write "下一輪交接摘要" --type handoff --json
agentx memory-write "已確認事實" --tier human_confirmed --type fact --write --json
agentx status --json
...
agentx tool-plan write_file --args-json '{"path":".agentx/state.json","content":"{}"}' --json --fail-on-blocker
...
`agentx command-plan COMMAND --json` 會輸出 `agentx.command_plan.v1` command policy preflight，判斷命令是否符合 GREEN allowlist、YELLOW build approval、docker compose policy、top-level agentX CLI capability、headless agent posture 或 destructive blocker；它不會執行命令，且 headless 命令會回報 artifact/session/output-format 等 `tool_args`。
...
`agentx gate --json` 會輸出 `agentx.gate.v1` aggregate runner gate，聚合 review、doctor static 與 latest approvals audit；它用 denied approval、doctor failure、review blocker 擋流程，並把 doctor 的 workflow artifact health warning 一併帶出，`--fail-on-blocker` 可供 CI/wrapper 使用。
`agentx next --json` 會輸出 `agentx.next.v1` deterministic next-step planner，根據 diff、active tasks、latest artifacts、denied approvals 推薦下一個 runner command；latest workflow-run artifact 會直接導向 `workflow-inspect` 或 `workflow-resume --result-output auto --dry-run`，讓 ready workflow 續跑時自動接上下一個 artifact；每個 recommendation 會附 `command_plan`，且它不跑測試、不呼叫 LLM、不 mutate。
`agentx init --json` 會輸出 `agentx.init.v1` project profile；預設 read-only，加 `--write-memory` 才會寫入 Memory Hall。
`agentx memory-status --json` 會輸出 `agentx.memory_status.v1`，read-only 檢查 Memory Hall / AMH backend posture、AMH CLI 可用性、store/path 與 token set/missing；加 `--live-probe` 才會跑本機 AMH CLI probe。
...
`agentx doctor --json` 會輸出 `agentx.doctor.v1` health checks；CI 或 wrapper 可用 `agentx doctor --static --json` 只檢查本機 `uv`、git、task migration，避開 Ollama / memory live probes，並附 `workflow_artifact_health` 顯示 latest workflow-run artifact chain 是否 ready 或 needs_inspect。加上 `--fail-on-error` 時，任一 check 失敗會在輸出 payload 後以 exit 1 結束。
...
`agentx workflow-run NAME --json` 會輸出 `agentx.workflow_run.v1`，預設 dry-run 並包住 workflow-plan；只有加 `--execute` 才會逐步執行 eligible GREEN `agentx` CLI steps，遇到 placeholder blocker、slash step、YELLOW/RED gate 或命令失敗就停止；若要執行 YELLOW gate，必須同時加 `--allow-yellow-gates` 與 `--approval-reason TEXT`，payload 會回傳 `approval_receipts`。加 `--result-output PATH` 可把同一份 result payload 寫到 workspace 內 artifact，拒絕覆蓋既有檔案或寫到 workspace 外；`--result-output-format auto|json|jsonl` 可指定 artifact 格式，`auto` 會跟隨 stdout 的 JSON/JSONL 型態。這類 artifact 會被 `agentx artifacts`、`agentx next` 與 `agentx inspect` 發現。
`agentx workflow-inspect PATH --json` 會讀取已保存的 `agentx.workflow_run.v1` artifact，輸出 `agentx.workflow_artifact.v1`，包含原 workflow 狀態、缺失 input、已知 input、`resume_command` / `resume_argv`；可重複加 `--input KEY=VALUE` 在 inspect 階段補齊 placeholder。`agentx workflow-resume PATH --dry-run --json` 會輸出 `agentx.workflow_resume.v1`，用於把上一輪 workflow-run artifact 轉回可執行的 `agentx workflow-run ...` 命令；也可加 `--input KEY=VALUE` 補齊缺失 input，或用 `--result-output auto` 自動配置下一個 `.agentx/runs/*-next.json/jsonl` artifact path。只有 `workflow-resume --execute` 會執行生成命令；JSON/JSONL 模式會回傳 execution cwd、returncode、stdout/stderr。
...
`--output-format jsonl` 會輸出單行 event envelope，例如 `{"event":"result","data":{...}}`；dry-run、version、backends、capabilities、instructions、inspect、diff、patch-check、command-plan、review、commit-plan、gate、next、tool-plan、infra、ace-init、ace-append、ace-briefing、ace-answer、ace-status、config、memory-status、memory-read、memory-write、sessions、artifacts、handoff-inspect、handoff-resume、approvals、traces、tasks、task-update、verify、status、doctor、commands、workflows、workflow-plan、workflow-run、workflow-inspect、workflow-resume、tools、models 會分別使用 `dry_run`、`version`、`backends`、`capabilities`、`instructions`、`inspect`、`diff`、`patch_check`、`command_plan`、`review`、`commit_plan`、`gate`、`next`、`tool_plan`、`infra`、`ace_init`、`ace_append`、`ace_briefing`、`ace_answer`、`ace_status`、`config`、`memory_status`、`memory_read`、`memory_write`、`sessions`、`artifacts`、`handoff_inspect`、`handoff_resume`、`approvals`、`traces`、`tasks`、`task_update`、`verify`、`status`、`doctor`、`commands`、`workflows`、`workflow_plan`、`workflow_run`、`workflow_inspect`、`workflow_resume`、`tools`、`models` event。
...
headless run 穩定欄位契約見 [`docs/HEADLESS_PAYLOAD_CONTRACT.md`](docs/HEADLESS_PAYLOAD_CONTRACT.md)；inspect/catalog/status 類 CLI JSON 契約見 [`docs/CLI_JSON_CONTRACTS.md`](docs/CLI_JSON_CONTRACTS.md)。
...
| Command | 說明 |
|---|---|
| `/help [COMMAND]` | 列出所有 slash command；提供 COMMAND 時顯示 usage、examples、risk 與 related commands |
| `/commands [QUERY]` | 列出或搜尋 slash command catalog；QUERY 支援命令 prefix 或 keyword |
| `/guide` | 60 秒快速導覽：模式選擇、常用工作流、安全與記憶 |
| `/workflows` | 列出理解、修改、測試、review、handoff、headless resume、approval audit 等實務路徑 |
...
| `/tools [QUERY]` | 列出或搜尋 agent 模式可用工具；QUERY 支援 keyword 或 GREEN/YELLOW/RED |
...
| `/sessions` | 列出最近 transcript 摘要與 approval receipt counts，可搭配 `/resume` |
| `/transcript [approvals [latest\|SESSION] [--denied]]` | 顯示 JSONL transcript 路徑；`approvals` 會列出本輪、最近上一輪或指定 session 的 YELLOW approval receipts，可用 `--denied` 只看拒絕項 |
...
| `/infra [all\|quick\|project\|resource\|home\|vps\|resource-bundle]` | 讀取專案/資源地圖；`home`/`vps` 會抽取家庭 AI 設施與 VPS 章節，`resource-bundle` 與 `資源地圖+家庭AI設施/VPS地圖` 會載入 resource + home + vps（read-only）。地圖分層與使用規則見 [`docs/RESOURCE_MAPS.md`](docs/RESOURCE_MAPS.md) |
...
每個核心 slash command 都可用 `/help COMMAND` 查 usage、examples、risk 與 related commands，也可在 shell 內用 `/commands QUERY` 搜尋 catalog。這些 metadata 由單一 command catalog 生成；外部 TUI、文件生成器或上游 agent 可用 `agentx commands --json` 取得同一份機器可讀 catalog，也可用 `agentx commands /workflow --json` 或 `agentx commands memory --json` 直接查命令 prefix / keyword。工具清單與風險 metadata 可用 `/tools` 或 `agentx tools --json` 取得，也可用 `/tools git`、`/tools YELLOW`、`agentx tools git --json` 或 `agentx tools YELLOW --json` 依 keyword / risk 聚焦。輸入未知 slash command 時，agentX 會提示最接近的候選命令，例如 `/wrkflow` 會建議 `/workflow`。
