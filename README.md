...
`agentx next --json` 會輸出 `agentx.next.v1` deterministic next-step planner，根據 diff、active tasks、latest artifacts、denied approvals 推薦下一個 runner command；latest workflow-run artifact 會直接導向 `workflow-inspect` 或 `workflow-resume --dry-run`；每個 recommendation 會附 `command_plan`，且它不跑測試、不呼叫 LLM、不 mutate。
