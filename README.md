...
每個核心 slash command 都可用 `/help COMMAND` 查 usage、examples、risk 與 related commands。這些 metadata 由單一 command catalog 生成。輸入未知 slash command 時，agentX 會提示最接近的候選命令，例如 `/wrkflow` 會建議 `/workflow`。
