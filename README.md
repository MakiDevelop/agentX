...
`agentx workflows --json` 會輸出 `agentx.workflow_catalog.v1`，讓 wrapper 能讀取 headless、audit、commit 等 recipe；每個 workflow 會提供 `steps` 與可直接處理的 `commands`；也可用 `agentx workflows headless --json` 查單一路徑。
