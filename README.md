...
`agentx workflow-run NAME --json` 會輸出 `agentx.workflow_run.v1`，預設 dry-run 並包住 workflow-plan；只有加 `--execute` 才會逐步執行 eligible GREEN `agentx` CLI steps，遇到 placeholder blocker、slash step、YELLOW/RED gate 或命令失敗就停止。
...
`--output-format jsonl` 會輸出單行 event envelope，例如 `{"event":"result","data":{...}}`；dry-run、version、backends、capabilities、instructions、inspect、diff、patch-check、command-plan、review、commit-plan、gate、next、tool-plan、infra、ace-init、ace-append、ace-briefing、ace-answer、ace-status、config、memory-status、memory-read、memory-write、sessions、artifacts、handoff-inspect、handoff-resume、approvals、traces、tasks、task-update、verify、status、doctor、commands、workflows、workflow-plan、workflow-run、tools、models 會分別使用 `dry_run`、`version`、`backends`、`capabilities`、`instructions`、`inspect`、`diff`、`patch_check`、`command_plan`、`review`、`commit_plan`、`gate`、`next`、`tool_plan`、`infra`、`ace_init`、`ace_append`、`ace_briefing`、`ace_answer`、`ace_status`、`config`、`memory_status`、`memory_read`、`memory_write`、`sessions`、`artifacts`、`handoff_inspect`、`handoff_resume`、`approvals`、`traces`、`tasks`、`task_update`、`verify`、`status`、`doctor`、`commands`、`workflows`、`workflow_plan`、`workflow_run`、`tools`、`models` event。
