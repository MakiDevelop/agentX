...
`agentx inspect --json` 會輸出 `agentx.inspect.v1` read-only preflight bundle，彙整 status、active tasks、sessions、latest approvals、latest traces、diff、capabilities、可跑的 verify commands 與對應 `command-plan`，但不執行測試或 live probes。
