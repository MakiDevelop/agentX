...
`agentx workflow-inspect PATH --json` 會讀取已保存的 `agentx.workflow_run.v1` artifact，輸出 `agentx.workflow_artifact.v1`，包含原 workflow 狀態、缺失 input、已知 input、`resume_command` / `resume_argv`；`agentx workflow-resume PATH --dry-run --json` 會輸出 `agentx.workflow_resume.v1`，用於把上一輪 workflow-run artifact 轉回可執行的 `agentx workflow-run ...` 命令。只有 `workflow-resume --execute` 會執行生成命令。
