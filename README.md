...
每個核心 slash command 都可用 `/help COMMAND` 查 usage、examples、risk 與 related commands。這些 metadata 由單一 command catalog 生成；外部 TUI、文件生成器或上游 agent 可用 `agentx commands --json` 取得同一份機器可讀 catalog。輸入未知 slash command 時，agentX 會提示最接近的候選命令，例如 `/wrkflow` 會建議 `/workflow`。
